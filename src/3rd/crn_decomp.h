...
typedef unsigned int    crn_crnd_bool;
...
#define CRND_RESTRICT __restrict
...
#ifdef _MSC_VER
   #if _MSC_VER < 1300
   static void crnd_stb_barrier(void)
   {
      #ifdef MemoryBarrier
      MemoryBarrier();
      #else
   ...
   #define CRND_WRITE_BARRIER crnd_stb_barrier();
   #define CRND_FULL_BARRIER crnd_stb_barrier();
   #else
   #include <intrin.h>
...
// first delete all the code that causes problems compiling in VC6
      ...
      inline bool resize(uint32 new_size)
      {
         if (m_size != new_size)
         {
            ...
            else
            {
               ...
               memset(m_p + m_size, 0, sizeof(*m_p) * (new_size - m_size));
            }
